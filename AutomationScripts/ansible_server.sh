...
logFilePath=/home/${userName}/ansibleServer_$(date +%y%m%d).log
...
printf "All Packages Installed." >> ${logFilePath}
...
cd .ssh
echo "yes" | ssh-copy-id ansible@${host1_pvt_ip}
sshpass -p "anisble1234" ssh-copy-id ansible@${host1_pvt_ip}
echo "yes" | ssh-copy-id ansible@${host2_pvt_ip}
sshpass -p "anisble1234" ssh-copy-id ansible@${host2_pvt_ip}
echo "Loading Key-Pairs to Target Servers : COMPLETED" >> ${logFilePath}
echo "Ansible-Server : Setup Completed" >> ${logFilePath}
echo "Ansible-Server : Setup Completed"
